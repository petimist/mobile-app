...
const mockInput = {
    ...
    'wp:featuredmedia': [
      {
        media_details: {
          sizes: {
            full: {
              source_url:
                'https://www.digitalsupply.co/wp-content/uploads/2018/03/glacier-blue.jpg',
            },
          },
        },
      },
  ...
  link:
    'https://www.digitalsupply.co/using-open-source-software-to-build-your-instagram-followers/',
};
...
it('Articles fetchList() returns correctly', async () => {
  ...
  await model
    .effects(dispatch)
    .fetchList({ page: 2 }, initialState)
    .then((res) => {
      expect(Api.get).toHaveBeenCalledWith(
        '/v2/posts?per_page=4&page=2&orderby=modified&_embed'
      );
...
it('Articles Model Save returns correctly', () => {
  expect(
    model.reducers.replaceUserInput(
      {
        meta: {
          lastPage: 2,
          total: 2,
